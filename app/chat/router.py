...
from app.users.dependensies import get_current_user, get_dao
...
async def get_chat_page(request: Request, user_data: User = Depends(get_current_user), dbs: tuple[UsersDAO, MessagesDAO] = Depends(lambda: get_dao(ENVIRONMENT))):
...
async def get_messages(user_id: int, current_user: User = Depends(get_current_user), dbs: tuple[UsersDAO, MessagesDAO] = Depends(lambda: get_dao(ENVIRONMENT))):
...
async def send_message(message: MessageCreate, current_user: User = Depends(get_current_user), dbs: tuple[UsersDAO, MessagesDAO] = Depends(lambda: get_dao(ENVIRONMENT))):
    UsersDAO, MessagesDAO = dbs
